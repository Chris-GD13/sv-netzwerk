...
new_to = '''<label>An<div class="vf-mail-recipient-row"><input id="vf-mail-to" type="email" placeholder="Empfänger"><select id="vf-mail-preset" aria-label="Standardempfänger auswählen"><option value="">Standardempfänger</option><option value="service.schaden@sparkassenversicherung.de">Sparkassenversicherung – Schadenservice</option><option value="archiv@sparkassenversicherung.de">Sparkassenversicherung – Archiv</option><option value="backoffice@meygeneralbau.de">Mey Generalbau – Backoffice</option><option value="info@rainbow-sanierungen.de">Rainbow Sanierungen – Zentrale</option><option value="service@polygon-deutschland.de">POLYGON Deutschland – Service</option></select></div></label>'''
...
# Auswahl bei bereits gepatchter Oberfläche ergänzen.
existing_service = '<option value="service.schaden@sparkassenversicherung.de">Sparkassenversicherung – Schadenservice</option>'
options = [
    '<option value="archiv@sparkassenversicherung.de">Sparkassenversicherung – Archiv</option>',
    '<option value="backoffice@meygeneralbau.de">Mey Generalbau – Backoffice</option>',
    '<option value="info@rainbow-sanierungen.de">Rainbow Sanierungen – Zentrale</option>',
    '<option value="service@polygon-deutschland.de">POLYGON Deutschland – Service</option>',
]
if 'id="vf-mail-preset"' in updated:
    if existing_service not in updated:
        raise SystemExit('Standardempfänger-Auswahl gefunden, Schadenservice-Option fehlt jedoch')
    insertion = existing_service
    for option in options:
        if option not in updated:
            insertion += option
    if insertion != existing_service:
        updated = updated.replace(existing_service, insertion, 1)
