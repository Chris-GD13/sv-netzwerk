(()=>{
  ...
  button.addEventListener('click',async()=>{
    if(userJobs.length)return;
    if(context.claims_agent&&!bridge){show(`Diese zentrale Importstation ist nicht bereit. Browser-Brücke 1.3.12 erforderlich (geladen: ${bridgeVersion||'nicht erkannt'}). Bitte zuerst die Brücke aktualisieren.`,true);return}
  ...
  window.addEventListener('message',async e=>{
    ...
    if(d.type==='SVNET_CLAIMS_BRIDGE_READY'){
      ...
      bridge=versionAtLeast(bridgeVersion,'1.3.12');
      if(!bridge)show(`Browser-Brücke 1.3.12 erforderlich (geladen: ${bridgeVersion}). Bitte die Erweiterung aktualisieren.`,true);
    }
