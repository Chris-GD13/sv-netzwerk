...
$documentType=(string)($data['document_type']??'settlement');
$isKvaReview=$documentType==='kva_review';
...
$kvaGross=(float)($kva['gross_total']??0);
if($isKvaReview&&($kvaCompany===''||$kvaNumber===''||$kvaGross<=0)){http_response_code(400);echo 'KVA-Firma, KVA-Nummer oder KVA-Gesamtbetrag fehlt';exit;}
$documentTopLabel=$isKvaReview?'KVA-PRÜFUNG':'ABGELTUNGSVEREINBARUNG';
$documentTitle=$isKvaReview?'KVA-Prüfung':'Abgeltungsvereinbarung';
$documentSubtitle=$isKvaReview?'Kostenvoranschlag mit BKI-Preisen nachkalkuliert':'Schadenkalkulation als Grundlage der Regulierung';
$tableLastHeader=$isKvaReview?'Prüfung':'Regulierung';
...
function drawPageHeader(array &$ops,bool $continuation=false):float{global $navy,$orange,$muted,$lineColor,$documentTopLabel;drawLogo($ops);rightText($ops,550,799,$documentTopLabel,8,true,$navy);if($continuation)rightText($ops,550,787,'Fortsetzung',7,false,$muted);strokeLine($ops,45,775,550,775,$lineColor,.7);fillRect($ops,45,771,72,3,$orange);return 752;}
function drawTableHeader(array &$ops,float &$y):void{global $navy,$panel,$tableLastHeader;fillRect($ops,45,$y-20,505,20,$panel);textLine($ops,50,$y-13,'Pos.',7.5,true,$navy);textLine($ops,78,$y-13,'Leistung',7.5,true,$navy);textLine($ops,318,$y-13,'Grundlage',7.5,true,$navy);rightText($ops,462,$y-13,'Betrag brutto',7.5,true,$navy);textLine($ops,472,$y-13,$tableLastHeader,7.5,true,$navy);$y-=23;}
...
textLine($ops,45,$y,$documentTitle,19,true,$navy);$y-=18;
textLine($ops,45,$y,$documentSubtitle,9,false,$muted);$y-=22;
...
foreach($lines as$line){
  ...
  $regulation=$isKvaReview?['BKI-Nachkalkulation']:($mode==='percent'?[numberValue($percent).' % vom Netto',money($settlementAmount)]:($mode==='full'?['Auszahlung 100 %',money($settlementAmount)]:['Wiederherstellung']));
...
if($isKvaReview){needSpace($pages,$ops,$y,70,false);$y-=24;textLine($ops,45,$y,'Geprüft:',8,false,$muted);$y-=16;textLine($ops,45,$y,$regulator,10,true,$navy);$y-=14;textLine($ops,45,$y,'Sachverständiger & Regulierer',8,false,$muted);}else{needSpace($pages,$ops,$y,105,false);$y-=28;strokeLine($ops,50,$y,245,$y,$blue,.8);strokeLine($ops,350,$y,545,$y,$blue,.8);$y-=15;textLine($ops,50,$y,'Versicherungsnehmer / Bevollmaechtigter',7.5,true,$navy);textLine($ops,350,$y,$regulator,8.5,true,$navy);$y-=12;textLine($ops,350,$y,'Regulierer',7.5,false,$muted);}if($ops)$pages[]=$ops;
...
$filename=preg_replace('/[^A-Za-z0-9._-]+/','-',$caseNo!==''?$caseNo:'Schaden').($isKvaReview?'_KVA-Pruefung_':'_Abgeltungsvereinbarung_').$date.'.pdf';header('Content-Type: application/pdf');header('Content-Disposition: attachment; filename="'.$filename.'"');header('Content-Length: '.strlen($pdf));echo$pdf;
