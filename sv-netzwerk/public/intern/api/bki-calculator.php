...
function bkArchiveHtml(array $case,array $items,array $totals,string $location,string $note,int $calcId):string{
  ...
  foreach($items as $index=>$item){
    ...
    if(($item['type']??'')==='section'){
      $rows.='<tr class="section"><td colspan="9">'.$h($item['description']??'KVA-Positionen').'</td></tr>';
      continue;
    }
    $position++;
    ...
    $rows.='<tr>'
      .'<td>'.$position.'</td>'
      .'<td>'.$h($item['source_position_code']??'').'</td>'
  ...
  return '<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    .'<title>'.$h($title).'</title><style>body{font-family:Arial,sans-serif;color:#17324a;margin:28px;font-size:13px}h1{margin:0 0 6px}p{line-height:1.45}.meta{background:#f3f6f8;padding:12px;border-radius:8px;margin:14px 0}table{width:100%;border-collapse:collapse;margin-top:16px}th,td{border:1px solid #d7e0e7;padding:7px;vertical-align:top}th{background:#eef3f6;text-align:left}.section td{background:#eef4f7;border-top:2px solid #17324a;font-weight:bold}.num{text-align:right;white-space:nowrap}.totals{margin-left:auto;width:320px;margin-top:16px}.totals td:first-child{font-weight:bold}.gross{font-size:15px;font-weight:bold}.note{margin-top:18px;padding:12px;border-left:4px solid #ff970f;background:#fff8ed}.source{margin-top:18px;font-size:11px;color:#65788a}@media print{body{margin:12mm}.source{page-break-inside:avoid}}</style></head><body>'
    ...
    .'<table><thead><tr><th>Pos.</th><th>Ursprungs-Pos.</th><th>Leistung</th><th>Menge</th><th>Einheit</th><th>EP netto</th><th>Reg.-Faktor</th><th>Gesamt netto</th><th>Quelle</th></tr></thead><tbody>'.$rows.'</tbody></table>'
