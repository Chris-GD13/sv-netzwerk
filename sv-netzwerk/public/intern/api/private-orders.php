...
if ($action==='create' && $_SERVER['REQUEST_METHOD']==='POST') { $input=json_decode((string)file_get_contents('php://input'),true);$name=trim((string)($input['name']??''));$parent=trim((string)($input['parent_id']??$root['id']));if($name===''||mb_strlen($name)>120||preg_match('/[\\\/\x00-\x1F]/u',$name))apiError(400,'Bitte einen gültigen Auftragsnamen eingeben.');$tree=poTree((string)$root['id']);if($parent!==(string)$root['id']&&!poFind($tree,$parent))apiError(403,'Der Zielordner gehört nicht zum eigenen Privatauftragsbereich.');$created=poRequestJson('POST','https://graph.microsoft.com/v1.0/drives/'.rawurlencode(poDriveId()).'/items/'.rawurlencode($parent).'/children',['name'=>$name,'folder'=>new stdClass(),'@microsoft.graph.conflictBehavior'=>'rename']);foreach(['Auftrag','Rechnungen','Angebot','Gutachten','Dateien','Bilder','E-Mails','Eingang KI'] as $sub)poRequestJson('POST','https://graph.microsoft.com/v1.0/drives/'.rawurlencode(poDriveId()).'/items/'.rawurlencode((string)($created['id']??'')).'/children',['name'=>$sub,'folder'=>new stdClass(),'@microsoft.graph.conflictBehavior'=>'fail']);apiJson(['ok'=>true,'item'=>$created]);}
