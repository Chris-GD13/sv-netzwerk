...
function gfCalculationAssessmentRule(): string
{
    return <<<'TEXT'

Wenn ein Kostenvoranschlag zu prüfen ist, muss summary als kopierfertige fachliche KVA-Bewertung in deutscher Sprache aufgebaut sein. Die Reihenfolge ist verbindlich: (1) kurze Gesamtaussage mit angebotener und maximal vertretbarer Netto-/Bruttosumme, (2) Mengenermittlung mit Anzahl, Einzelabmessungen, Rechenwegen und Gesamtsumme, (3) eigene Nachkalkulation mit Material, Nebenmaterial, Transport, Vorbereitung/Zuschnitt, Arbeitszeit sowie Entsorgung und nachvollziehbaren Einzelansätzen, (4) Gegenüberstellung von KVA und Nachkalkulation jeweils netto und brutto einschließlich Differenzen und prozentualer Abweichung, (5) technische und wirtschaftliche Bewertung der wesentlichen Abweichungen, insbesondere Material- und Facharbeiterstunden, und (6) eine unmittelbar verwendbare Regulierungsempfehlung als vollständiger Textblock. Sämtliche Multiplikationen, Netto-, Umsatzsteuer- und Bruttosummen sowie Differenzen rechnerisch prüfen. Mengen aus Stückzahl und Abmessungen selbst berechnen, wenn diese Angaben in der Akte eindeutig belegt sind. Weiterverwendete Bauteile und Profile dürfen nicht als Erneuerungsleistung angesetzt werden. Preise ausschließlich aus den bereitgestellten BKI-Unterlagen oder konkret belegten KVA-, Rechnungs- und Marktpreisunterlagen verwenden; Internetpreise oder Anbieter niemals erfinden. Fehlen trotz vollständiger Auswertung einzelne Mengen oder Preise, die betroffene Position im bearbeitbaren Entwurf belassen, eindeutig als manuell zu ergänzen kennzeichnen und in open_points nennen; deswegen weder die gesamte Kalkulation noch weitere ausgewählte Dokumente abbrechen. Keine unbelegte Menge einsetzen.
TEXT;
}
...
function gfPlanCalculationFromEvidence(string $evidence, array $meta, string $instructions): array
{
    ...
    $system .= gfCalculationAssessmentRule();
...
function gfEnsureRequiredWaterCalculationBlocks(array $result, string $evidence, array $meta, string $instructions, string $storeId): array
{
    ...
    $repair = gfCalculationOpenAIRequest([
        ...
        'input' => gfCalculationAssessmentRule()."\nFalldaten:\n".json_encode($meta, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
            ."\n\nArbeitsauftrag:\n".$instructions
            ."\n\nAktenauszug:\n".$evidence
