 ...
 * POST   (multipart) ?window_id={id} – Foto hochladen
...
match (true) {
    ...
    $method === 'POST'   && $wid !== null => handleUpload($wid, $user),
...
function handleUpload(int $windowId, array $user): never
{
    ...
    try {
        db()->prepare(
            'INSERT INTO photos
             (window_id, category, caption, file_name, storage_path, inspector_id, inspector_name, taken_at, created_at)
             VALUES (:wid, :cat, :cap, :fn, :sp, :iid, :in, :now, :now2)'
        )->execute([
            ':wid'  => $windowId,
            ':cat'  => $category,
