...
// Die Originale werden einmal je Auftrag in Gruppen gelesen und anschließend
// als vollständiger, dateibezogener Evidenzbestand an alle Ausgaben übergeben.
$sourceNamesNeedle = '$sourceNames=array_map(fn($r)=>(string)$r[\'name\'],$caseRefs);';
$sourceNamesReplacement = <<<'PHP_CODE'
$sourceNames=array_map(fn($r)=>(string)$r['name'],$caseRefs);$caseEvidence=[];$caseChunks=array_chunk($caseRefs,10);foreach($caseChunks as$chunkIndex=>$caseChunk){gfJobUpdate($jobId,'running',18+min(18,(int)floor((($chunkIndex+1)/max(1,count($caseChunks)))*18)),'Fallunterlagen werden vollständig ausgewertet (Gruppe '.($chunkIndex+1).'/'.count($caseChunks).').');$evidenceContent=[['type'=>'input_text','text'=>'Lies jede beigefügte Datei vollständig und erstelle ausschließlich JSON mit dieser Struktur: {"files":[{"name":"exakter Dateiname","document_type":"Dokumentart","facts":["jede relevante Tatsachenangabe mit Einheit und Kontext"],"amounts":[{"description":"Position oder Summe","net":null,"tax":null,"gross":null}],"measurements":[{"description":"Maß oder Fläche","value":"exakter Wert mit Einheit","measurement_type":"Fläche|Länge|Feuchte|Druck|sonstiges"}],"participants":["Name und Funktion"],"open_points":[]}]} . Erfinde nichts. Kostenvoranschläge und Rechnungen vollständig mit Aussteller, Nummer, Datum, sämtlichen wesentlichen Hauptleistungsgruppen sowie Netto-, Umsatzsteuer- und Bruttosumme erfassen. DIGITS und Feuchtemesswerte ausdrücklich als measurement_type Feuchte kennzeichnen und niemals als Fläche. Jede Datei muss genau einmal im files-Array erscheinen.'];foreach($caseChunk as$caseRef)$evidenceContent[]=['type'=>'input_file','file_id'=>$caseRef['file_id']];$caseEvidence[]=gfOpenAI($evidenceContent,'Du extrahierst vollständig und quellengetreu Tatsachen aus deutschen Schadenakten. Antworte ausschließlich als valides JSON. Keine Zusammenfassung über mehrere Dateien; jede Datei einzeln und vollständig erfassen.');}
PHP_CODE;
$source = str_replace($sourceNamesNeedle, trim($sourceNamesReplacement), $source, $countCaseEvidence);
if ($countCaseEvidence !== 1) throw new RuntimeException('Vollständige Aktenauswertung konnte nicht sicher angebunden werden.');
...
if($key==='erstbericht'){$checkedHergang='';$checkedUmfang='';$checkedKalkulation='';foreach(($result['sections']??[])as$checkedIndex=>$checkedSection){$checkedHeading=gfNorm((string)($checkedSection['heading']??''));$checkedText=preg_replace('/\bHer(?=\s+[A-ZÄÖÜ])/u','Herr',(string)($checkedSection['text']??''))??(string)($checkedSection['text']??'');$result['sections'][$checkedIndex]['text']=$checkedText;if($checkedHeading==='schadenhergang')$checkedHergang=$checkedText;if($checkedHeading==='schadenumfang')$checkedUmfang=$checkedText;if($checkedHeading==='kalkulation')$checkedKalkulation=$checkedText;}if(preg_match('/\b(?:Makler|Agentur|Vermittler)\b/ui',$instructions)&&!preg_match('/\b(?:Makler|Agentur|Vermittler)\b/ui',$checkedHergang)){$broker=trim((string)($meta['vermittler_firma']??$meta['vermittler_ansprechpartner']??''));if(preg_match('/[@0-9]/u',$broker))$broker='';$vnContact=trim((string)($meta['kontakt']??''));if(preg_match('/[@0-9]/u',$vnContact))$vnContact='';$vnParticipant=$vnContact!==''?$vnContact.' für den Versicherungsnehmer':'der Versicherungsnehmer beziehungsweise dessen Vertretung';$brokerParticipant=$broker!==''?$broker.' als betreuender Vermittler':'der betreuende Makler';$participantText='Am aktuellen Ortstermin nahmen '.$vnParticipant.' sowie '.$brokerParticipant.' teil.';foreach(($result['sections']??[])as$participantIndex=>$participantSection)if(gfNorm((string)($participantSection['heading']??''))==='schadenhergang'){$existing=trim((string)($result['sections'][$participantIndex]['text']??''));$result['sections'][$participantIndex]['text']=trim($existing.' '.$participantText);break;}}$checkedFullText=gfEngelText($result);if(preg_match('/\b(?:DIGITS?|Feuchtemesswerte?)\b/ui',$checkedFullText))throw new RuntimeException('Allgemeiner Erstbericht gesperrt: DIGITS beziehungsweise einzelne Feuchtemesswerte dürfen im Bericht nicht ausgegeben werden.');if(preg_match('/\b(?:nach\s+den\s+vorliegenden|laut|gemäß|anhand|aus\s+dem|im\s+(?:Angebot|KVA)|belegt|übernommen)\b[^.]{0,100}\b(?:m²|qm|lfm)\b/ui',$checkedUmfang))throw new RuntimeException('Allgemeiner Erstbericht redaktionell unzulässig: Herkunftshinweis im Aufmaß erkannt.');$checkedHasKva=preg_match('/\b(?:KVA|Kostenvoranschlag|Angebot)\b/ui',implode(' ',array_map('strval',$sourceNames)))===1;if($checkedHasKva&&(!preg_match('/(?:EUR|€)/u',$checkedKalkulation)||preg_match('/\b(?:kein|keine|nicht)\b[^.]{0,80}\b(?:KVA|Kostenvoranschlag|Angebot)\b/ui',$checkedKalkulation)))throw new RuntimeException('Allgemeiner Erstbericht gesperrt: vorhandener Kostenvoranschlag wurde in der Kalkulation nicht mit Beträgen ausgewertet.');}
$contentQs=gfEngelValidate($key,$result,$meta,$instructions);
PHP_CODE;
