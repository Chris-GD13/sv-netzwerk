...
gfJobUpdate($jobId,'running',8,'Fallunterlagen und Schadenbilder werden vollständig ausgewertet.');$caseEvidence=[];$processedCaseFiles=0;$caseChunks=array_chunk($caseFiles,4);foreach($caseChunks as$chunkIndex=>$caseChunk){$chunkSignature=array_map(static fn(array$f):array=>['id'=>(string)($f['id']??''),'modified'=>(string)($f['modifiedTime']??''),'name'=>(string)($f['name']??''),'mime'=>(string)($f['mimeType']??'')],$caseChunk);$cacheKey='gf_evidence_v3_'.hash('sha256',json_encode($chunkSignature,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));$cachedEvidence=json_decode(gfSettingGet($cacheKey,'{}'),true);if(is_array($cachedEvidence)&&is_array($cachedEvidence['files']??null)&&count($cachedEvidence['files'])>0){$caseEvidence[]=$cachedEvidence;$processedCaseFiles+=count($caseChunk);gfJobUpdate($jobId,'running',8+min(18,(int)floor((($chunkIndex+1)/max(1,count($caseChunks)))*18)),'Fallunterlagen aus Zwischenspeicher übernommen: '.$processedCaseFiles.' von '.count($caseFiles).'.');continue;}$chunkRefs=[];foreach($caseChunk as$caseFile){gfJobUpdate($jobId,'running',8+min(17,(int)floor(($chunkIndex/max(1,count($caseChunks)))*18)),'Fallunterlage wird vorbereitet: '.($processedCaseFiles+count($chunkRefs)+1).' von '.count($caseFiles).'.');$caseRef=gfOpenAIUploadDriveFile($caseFile,'case');if($caseRef)$chunkRefs[]=$caseRef;}if(!$chunkRefs)continue;gfJobUpdate($jobId,'running',8+min(17,(int)floor(($chunkIndex/max(1,count($caseChunks)))*18)),'Dateigruppe '.($chunkIndex+1).' von '.count($caseChunks).' wird ausgewertet.');$evidenceContent=[['type'=>'input_text','text'=>'Lies jede beigefügte Datei beziehungsweise jedes Bild vollständig und erstelle ausschließlich JSON mit dieser Struktur: {"files":[{"name":"exakter Dateiname","document_type":"Dokumentart oder Schadenfoto","facts":["jede relevante Tatsachenangabe mit Einheit und Kontext"],"amounts":[{"description":"Position oder Summe","net":null,"tax":null,"gross":null}],"measurements":[{"description":"Maß oder Fläche","value":"exakter Wert mit Einheit","measurement_type":"Fläche|Länge|Feuchte|Druck|sonstiges"}],"visual_findings":["sichtbarer Schadenbereich, Bauteil, Raumzuordnung und erkennbarer Umfang"],"participants":["Name und Funktion"],"open_points":[]}]} . Erfinde nichts. Bilder einzeln auswerten: sichtbare Schäden, betroffene Bauteile, Raumzuordnung, Maßstäbe, Beschriftungen und nur tatsächlich ablesbare Maße erfassen. Keine verdeckten Schäden, Maße oder Mengen aus Perspektive oder Erfahrungswerten schätzen. Logos, Signaturen, Profilbilder und dekorative Bilder ignorieren. Kostenvoranschläge und Rechnungen vollständig mit Aussteller, Nummer, Datum, sämtlichen wesentlichen Hauptleistungsgruppen sowie Netto-, Umsatzsteuer- und Bruttosumme erfassen. Werte aus DIGITS oder sonstigen Feuchtemessungen nur intern als measurement_type Feuchte kennzeichnen; sie dürfen später weder als Fläche noch als Zahlenreihe in den Bericht gelangen. Jede Datei muss genau einmal im files-Array erscheinen.']];foreach($chunkRefs as$chunkRef)$evidenceContent[]=gfCalculationInputPart($chunkRef);$evidence=gfOpenAI($evidenceContent,'Du extrahierst vollständig und quellengetreu Tatsachen aus deutschen Schadenakten und Schadenbildern. Antworte ausschließlich als valides JSON. Keine Zusammenfassung über mehrere Dateien; jede Datei und jedes Bild einzeln und vollständig erfassen.',(int)env('OPENAI_EVIDENCE_MAX_OUTPUT_TOKENS','7000'));gfSettingSet($cacheKey,json_encode($evidence,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));$caseEvidence[]=$evidence;$processedCaseFiles+=count($caseChunk);gfJobUpdate($jobId,'running',8+min(18,(int)floor((($chunkIndex+1)/max(1,count($caseChunks)))*18)),'Fallunterlagen und Schadenbilder ausgewertet: '.$processedCaseFiles.' von '.count($caseFiles).'.');unset($chunkRefs,$evidenceContent,$evidence);}if(!$caseEvidence)throw new RuntimeException('Fallunterlagen konnten der KI nicht bereitgestellt werden.');gfJobUpdate($jobId,'running',28,'Regelwerke werden geladen.');
PHP_CODE;
