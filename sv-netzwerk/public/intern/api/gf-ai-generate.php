...
if($key==='erstbericht'){$generalText=gfEngelText($result);if(preg_match('/\b(?:vom\s+Bearbeiter|im\s+Arbeitsauftrag|im\s+System\s+(?:geführt|hinterlegt)|ist\s+(?:als\s+[^.]{0,80}\s+)?zu\s+übernehmen|anhand\s+der\s+Unterlagen\s+übernommen)\b/ui',$generalText))throw new RuntimeException('Allgemeiner Erstbericht redaktionell unzulässig: interne Bearbeitungs- oder Herkunftsformulierung erkannt.');if(preg_match('/\b(?:Gesamtansatz|Kostenschätzung)\b[^.]{0,100}\b0(?:,00)?\s*(?:EUR|€)/ui',$generalText)&&preg_match('/\b(?:Reserve|KVA|Kostenvoranschlag)\b[^.]{0,100}\b[1-9][0-9.]*,[0-9]{2}\s*(?:EUR|€)/ui',$generalText))throw new RuntimeException('Allgemeiner Erstbericht widersprüchlich: veralteter Nullansatz neben aktuellem Betrag erkannt.');if(preg_match('/\bTerminabgleich\b|\bTermin\s+mit\s+[^.]{0,80}\b(?:offen|abzustimmen)\b/ui',$generalText)&&!preg_match('/\bTermin(?:abgleich)?\b[^.]{0,100}\b(?:offen|abstimmen|vereinbaren|erforderlich)\b/ui',$instructions))throw new RuntimeException('Allgemeiner Erstbericht enthält einen nicht beauftragten zukünftigen Termin.');}
