...
const GF_CASES_FOLDER_ID = '1dqdQkccdGLuo8ijKerGgYdmNO2cBtD4P';
...
function gfDriveList(string $parentId): array {
    $q="'".str_replace("'","\\'",$parentId)."' in parents and trashed=false";$url='https://www.googleapis.com/drive/v3/files?'.http_build_query(['q'=>$q,'fields'=>'files(id,name,mimeType,modifiedTime,size,webViewLink)','pageSize'=>1000,'orderBy'=>'modifiedTime desc','supportsAllDrives'=>'true']);$r=gfHttp('GET',$url,[],null,true);if($r['status']!==200)apiError(503,'Google-Drive-Dateiliste konnte nicht geladen werden.');$d=json_decode($r['body'],true);return is_array($d['files']??null)?$d['files']:[];
}
...
apiJson(['ok'=>true,'created'=>$created,'count'=>count($created),'ai'=>true,'sources'=>$sourceNames]);
