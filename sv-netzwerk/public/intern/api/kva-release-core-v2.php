...
function krAnalyzeCalculation(string $name, string $mime, string $bytes): array
{
    ...
    try {
        ...
        $result = krOpenAiJson(
            $key,
            $fileId,
            'Lies den Kostenvoranschlag vollständig und positionsgenau. Erfinde keine Leistungen, Mengen, Einheiten oder Preise. Erfasse ausschließlich tatsächlich angebotene Hauptpositionen. Unterpositionen dürfen nur separat erscheinen, wenn sie einen eigenen Preis haben. Alternativ-, Eventual- und Bedarfspositionen kennzeichnest du und rechnest sie nicht in den angebotenen Gesamtbetrag ein. Geldbeträge werden als Dezimalzahlen ohne Währungszeichen ausgegeben. Prüfe quantity mal unit_price gegen line_total und nenne bei Abweichungen eine Warnung. Wenn nur eine eindeutige Nettosumme und ein ausdrücklicher Umsatzsteuersatz angegeben sind, dürfen vat_total und gross_total rechnerisch daraus abgeleitet werden; kennzeichne dies mit totals_basis=calculated_from_net_and_vat_rate. Positionstexte bleiben fachlich vollständig, werden aber ohne Kopf-/Fußzeilen übernommen. Lies den Empfänger/Auftraggeber und dessen vollständige Anschrift getrennt vom Aussteller. Formuliere außerdem eine knappe technische Plausibilitätsbewertung, aber bestätige ohne Schadenakte keine Schadenkausalität. Formuliere zusätzlich einen versandfertigen sachverständigen E-Mail-Entwurf. Der Entwurf folgt dieser Reihenfolge: formelle Anrede; geprüftes Angebot mit Anbieter, Nummer, Datum sowie Netto- und Bruttobetrag; nur wenn im Dokument positiv belegt ein Ursprungsangebot und dessen bereits berücksichtigter Betrag; Einordnung des angebotenen Wiederherstellungsumfangs und der geprüften Leistungen/Einheitspreise; eindeutige Befürwortung oder der fachlich erforderliche Vorbehalt mit Bruttobetrag; nur wenn positiv belegt die rechnerische Mehrung; abschließend der Hinweis, dass darüber hinausgehende Mehrleistungen vor Ausführung gesondert anzuzeigen und zur Prüfung vorzulegen sind. Verwende keine unbelegten Altdaten und erfinde weder Anrede noch Vorschadenberechnung. Nicht belegte Bausteine werden vollständig weggelassen; schreibe niemals, dass ein Ursprungsangebot, Betrag oder anderer Wert nicht belegt sei. Ist die Anrede nicht belegt, beginne neutral mit Guten Tag. Formatiere Datumsangaben als TT.MM.JJJJ und jeden Betrag deutsch mit zwei Nachkommastellen, Tausenderpunkt und Eurozeichen, zum Beispiel 2.639,00 € brutto.',
            'Datei: '.$name.'. Antworte als JSON mit company, company_email, company_street, company_postal_code, company_city, recipient_name, recipient_street, recipient_postal_code, recipient_city, project_location, quote_number, quote_date, net_total, vat_rate, vat_total, gross_total, totals_basis, technical_assessment, delimitation, release_recommendation, email_draft, positions (Array mit position_no, description, quantity, unit, unit_price, line_total, optional als Boolean und confidence zwischen 0 und 1), warnings. Unbekannte Werte als null.'
        );
        $positions = [];
        ...
        return [
            ...
            'email_draft'=>trim((string)($result['email_draft'] ?? '')),
            'positions'=>$positions,
