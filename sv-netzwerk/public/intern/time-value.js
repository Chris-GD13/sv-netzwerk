...
export function buildTimeValueText(component, result, source = {}) {
  ...
  return `Für das Bauteil „${component}“ wurde die technische Lebensdauer anhand des BTE-Arbeitsblatts „Lebensdauer von Bauteilen, Zeitwerte“ (Stand 14.03.2008${source.page ? `, Tabelle Seite ${source.page}` : ''}) mit ${result.lifetime} Jahren angesetzt.${lifetimeNote} ${methodNote} Ausgehend von einem Neuwert von ${money(result.newValueNet)} netto (${money(result.newValueGross)} brutto) ergibt sich ein Abzug „Neu für Alt“ von ${money(result.deductionNet)} netto (${money(result.deductionGross)} brutto). Der technische Zeitwert beträgt damit ${money(result.timeValueNet)} netto bzw. ${money(result.timeValueGross)} brutto. Die BTE-Werte sind Richtwerte; Beanspruchung, Qualität, Pflege, Unterhaltung, Nutzbarkeit und der konkrete Einzelfall wurden sachverständig zu würdigen.`;
}
