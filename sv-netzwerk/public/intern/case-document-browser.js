(()=>{
  ...
  const style=document.createElement('style');style.textContent=`
    #vf-doc-browser{margin:14px 0 18px;background:#fff;border:1px solid #d8e2e9;border-radius:14px;padding:14px 16px;color:#17324a}
    ...
    .vfdb-actions button,.vfdb-actions a{border:1px solid #b9c7d2;background:#fff;color:#17324a;border-radius:8px;padding:7px 10px;font-weight:700;text-decoration:none;cursor:pointer}.vfdb-actions .primary{background:#17324a;color:#fff;border-color:#17324a}
    .vfdb-search{width:100%;box-sizing:border-box;border:1px solid #bdcbd6;border-radius:8px;padding:9px 10px;margin:0 0 10px;font:inherit}
  ...
  const box=document.createElement('section');box.id='vf-doc-browser';box.hidden=true;box.innerHTML=`<div class="vfdb-head"><div><h3>Fallunterlagen</h3><small id="vfdb-meta">Unterlagen und Unterordner des aktiven Falls</small></div><div class="vfdb-actions"><button type="button" id="vfdb-refresh">Aktualisieren</button><button type="button" class="primary" id="vfdb-open-selected">Ausgewählte öffnen</button><a id="vfdb-drive" target="_blank" rel="noreferrer" hidden>Fallordner öffnen</a></div></div><input id="vfdb-search" class="vfdb-search" placeholder="Unterlagen durchsuchen …"><div id="vfdb-list" class="vfdb-list"></div>`;
  ...
  function renderItems(items,path=''){
    return (items||[]).map(item=>{
      if(item.folder){return `<details class="vfdb-folder" open data-name="${esc(item.name.toLowerCase())}"><summary>📁 ${esc(item.name)}</summary><div class="vfdb-folder-body">${renderItems(item.children||[],path+'/'+item.name)}</div></details>`;}
      const type=classify(item.name);const meta=[path.replace(/^\//,''),fmtDate(item.modifiedTime),fmtSize(item.size)].filter(Boolean).join(' · ');
      return `<div class="vfdb-file" data-name="${esc((item.name+' '+type+' '+path).toLowerCase())}"><input type="checkbox" data-url="${esc(item.webViewLink)}" aria-label="${esc(item.name)} auswählen"><a href="${esc(item.webViewLink)}" target="_blank" rel="noreferrer"><strong>${esc(type)} · ${esc(item.name)}</strong><small>${esc(meta)}</small></a><span class="vfdb-type">${esc(type)}</span></div>`;
    }).join('');
  }
  async function load(){const c=activeCase();if(!c?.folder_id){box.hidden=true;return;}box.hidden=false;$('vfdb-list').innerHTML='<div style="padding:12px">Fallunterlagen werden geladen …</div>';try{const r=await fetch('/intern/api/case-file-browser.php?folder_id='+encodeURIComponent(c.folder_id),{credentials:'same-origin'});const j=await r.json().catch(()=>({}));if(!r.ok||!j.ok)throw new Error(j.error||'Unterlagen konnten nicht geladen werden.');$('vfdb-list').innerHTML=renderItems(j.items||[]);$('vfdb-meta').textContent=`${j.folder?.name||'Fallakte'} · Unterlagen und Unterordner`;const a=$('vfdb-drive');if(j.folder?.webViewLink){a.href=j.folder.webViewLink;a.hidden=false;}else a.hidden=true;}catch(e){$('vfdb-list').innerHTML='<div style="padding:12px;color:#a33">'+esc(e.message||e)+'</div>';}}
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button,a');if(b?.textContent?.trim()==='Dokumente'){e.preventDefault();load();box.scrollIntoView({behavior:'smooth',block:'start'});}});
