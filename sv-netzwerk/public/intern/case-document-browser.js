(()=>{
  ...
  function renderItems(items,path=''){
    return (items||[]).map(item=>{
      if(item.folder){return `<details class="vfdb-folder" data-name="${esc(item.name.toLowerCase())}"><summary>📁 ${esc(item.name)}</summary><div class="vfdb-folder-body">${renderItems(item.children||[],path+'/'+item.name)}</div></details>`;}
