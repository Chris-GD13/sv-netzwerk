...
export async function buildSearchIndex(){
  const [knowledge,downloads,cases,videos]=await Promise.all([getCollection('knowledge'),getCollection('downloads'),getCollection('practiceCases'),getCollection('videos')]);
  const inputs:Input[]=[
    ...library,
    ...
    ...videos.filter((entry)=>entry.data.publication.status==='published').map((entry)=>({id:`video-${entry.id}`,title:entry.data.title,description:entry.data.description,href:`/wissen-in-180-sekunden/${entry.id}/`,category:'Wissen in 180 Sekunden',tags:[entry.data.damageType,...entry.data.tags],date:entry.data.publication.publishedAt.toISOString().slice(0,10),type:'video' as SearchContentType})),
