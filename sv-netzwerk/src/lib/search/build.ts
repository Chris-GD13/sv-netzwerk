...
export async function buildSearchIndex(){
  ...
  const inputs:Input[]=[
    ...library,
    ...knowledge.filter((entry)=>entry.data.publication.status==='published' && Boolean(entry.data.publication.publishedAt)).map((entry)=>({id:`knowledge-${entry.id}`,title:entry.data.title,description:entry.data.description,href:`/fachwissen/${entry.id}/`,category:entry.data.category,tags:entry.data.tags,date:entry.data.publication.publishedAt!.toISOString().slice(0,10),type:'article' as SearchContentType,featured:entry.data.featured})),
