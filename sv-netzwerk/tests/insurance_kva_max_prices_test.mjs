...
assert(page.includes('/intern/kva-calculation-import.js?v=20260910-4'),'Die Höchstpreis-Nachkalkulation muss mit der aktuellen Cache-Version geladen werden.');
...
assert(script.includes("modulePath='/vendor/pdfjs/pdf.mjs'")&&script.includes('import(modulePath)')&&script.includes("form.append('pages[]'")&&script.includes("form.append('page_numbers[]'")&&script.includes("parts=['oben','unten']")&&script.includes('case-file-browser.php?action=download'),'PDF-KVA aus Fallakte und Gerät müssen lokal als überlappende Seitenbilder gerendert werden, ohne den Renderer in die Astro-Prüfung zu ziehen.');
