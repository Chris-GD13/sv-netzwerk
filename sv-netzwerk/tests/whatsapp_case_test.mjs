...
for(const source of [client,accountClient])assert(source.includes('prepareSignup()')&&source.includes('signupConfig')&&source.includes('facebookSdk'),'Meta-Konfiguration und SDK müssen vor dem Klick geladen werden');
for(const source of [client,accountClient])assert(source.includes('const response=await new Promise(resolve=>FB.login(resolve')&&!source.includes('Der Meta-Anmeldedialog wurde blockiert.')&&!source.includes('const response=await Promise.race([new Promise(resolve=>FB.login'),'Eine laufende Meta-Bestätigung darf nicht nach einem starren Timeout verworfen werden');
