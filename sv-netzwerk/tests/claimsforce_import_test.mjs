...
assert(revenueApi.includes('RS_MARC_SOURCE_URL') && revenueApi.includes('function rsMarcSharePointSource()') && revenueApi.includes('function rsRefreshMarc(') && revenueApi.includes("rsPersist('claims_marc'") && revenueApi.includes("'profiles'=>['christian','claims_marc']"), 'Marcs Claims-Datei wird aus SharePoint automatisch eingelesen und getrennt gespeichert');
assert(revenueApi.includes('function rsMarcWarnings(') && settlementPage.includes('Hinweis zur Originaldatei'), 'Widersprüchliche Leistungszeiträume aus Marcs Quelle werden sichtbar gekennzeichnet und nicht still korrigiert');
