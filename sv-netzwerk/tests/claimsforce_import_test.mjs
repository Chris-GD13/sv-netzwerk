...
assert(revenueApi.includes("$action==='refresh'") && revenueApi.includes('Nur Susanne darf die Umsatzdatei manuell aktualisieren') && revenueApi.includes("$_FILES['workbook']") && settlementPage.includes('Jetzt aktualisieren') && settlementPage.includes("body.append('workbook',workbook,workbook.name)"), 'Susanne kann die gerade gespeicherte Umsatzdatei über einen eigenen Knopf zuverlässig neu einlesen');
...
assert(settlementPage.includes('settlement-scroll-top') && settlementPage.includes('holger-scroll-top') && settlementPage.includes('syncScrollWidths') && settlementPage.includes('connectScroll('), 'Obere synchronisierte horizontale Schiebeleisten machen die rechten Abrechnungsspalten beider Profile jederzeit erreichbar');
...
assert(settlementPage.includes("if(!next.visible){location.replace('/intern/versicherungsfaelle/')") && settlementPage.includes('refresh.hidden=!next.can_refresh'), 'Nicht berechtigte Nutzer werden abgewiesen und nur Susanne erhält die manuelle Aktualisierung');
