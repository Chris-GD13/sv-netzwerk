...
assert(internLayout.includes('class="intern-header__login" href="/intern/login/"') && internLayout.includes('Anmelden</a>'), 'Oben links im dunklen Kopfbereich ist dauerhaft ein direkter Anmeldebutton sichtbar');
assert(internLayout.includes("label: 'Fenstergutachten'") && !internLayout.includes("label: 'Fenster',"), 'Der Menüpunkt ist eindeutig als Fenstergutachten bezeichnet');
assert(calculationPage.indexOf('data-kva-import') < calculationPage.indexOf('bk-input-card'), 'Die KVA-Nachkalkulation steht als erste Arbeitskarte vor Suche und Schnellkalkulation');
...
assert(claimsPageBridge.includes('WITH_FUTURE_APPOINTMENT') && claimsPageBridge.includes('WITHOUT_APPOINTMENT') && claimsPageBridge.includes('observedClaims'), 'ClaimsForce-Planung mit und ohne Termin wird über Route und beobachtete API-Fallliste ausgelesen');
...
assert(options.includes("OPTIONS_CODE_VERSION = '1.4.11'"), 'Die Optionsseite kennzeichnet die aktuelle Brückenversion');
...
assert(drive.includes("$isBackoffice=gdIsSusanne($portalUser)") && drive.includes("$claimsAgent=$isBackoffice") && routing.includes("(string)($user['role'] ?? '') === 'administrator'") && routing.includes("$email === 'ks@sv-schuett.eu'"), 'Susanne, Katja und der Administratorzugang werden als zentrale Backoffice-Importstation erkannt');
...
assert(drive.includes('$indexed=searchCaseFolderIndex') && drive.includes("in_array((string)$root['id'],$folder['parents']??[],true)") && drive.includes('if($valid)return gdSortCaseSearchRows'), 'Lokale Indextreffer werden vor der Verwendung gegen den persönlichen Drive-Fallordner geprüft');
...
assert(central.includes("minimumBridgeVersion='1.4.11'") && central.includes('versionAtLeast(bridgeVersion,minimumBridgeVersion)'), 'Die zentrale Importstation darf nur mit der unmittelbar startenden und anforderungsbezogen gerouteten Brücke laufen');
...
assert(central.includes("currentBridgeVersion='1.4.11'") && central.includes('steht als empfohlenes Update bereit'), 'Die aktuelle Brücke muss weiterhin als empfohlenes Update angezeigt werden');
...
assert(identityWorker.includes('runningImport.portalTabId = portalTabId') && identityWorker.includes('resumed: true'), 'Ein wieder geöffneter identischer Import muss an die neue Portal-Registerkarte angebunden werden');
assert(identityWorker.includes('const portalTabId = () => Number(run.portalTabId || 0)') && identityWorker.includes('Number(run.portalTabId || 0)'), 'Portaloperationen und Abschlussmeldung müssen stets die zuletzt angebundene Import-Registerkarte verwenden');
...
assert(internLayout.includes("summary.summary_type === 'marc' || summary.summary_type === 'holger'") && internLayout.includes("summary.summary_type === 'marc' ? 'Marc' : 'Holger'"), 'Marc und Holger erhalten eine eindeutig personenbezogene Umsatzüberschrift');
...
assert(internLayout.includes('id="intern-settlement-link"') && internLayout.includes('/intern/abrechnungen/') && internLayout.includes('settlementLink.hidden = data.show_settlement_link === false'), 'Der geschützte Menüpunkt Abrechnungen wird nur nach bestätigter persönlicher Umsatzberechtigung sichtbar');
...
const portalServiceWorker = fs.readFileSync(path.join(root, 'public/sw.js'), 'utf8');
assert(revenueApi.includes('function rsCanViewProfile(') && revenueApi.includes('function rsCanRefreshProfile(') && revenueApi.includes("str_starts_with($profile,'rekon_')"), 'Claims- und Rekon-Abrechnungen besitzen getrennte rollenbezogene Lese- und Aktualisierungsrechte');
assert(revenueApi.includes("in_array($own,['christian','marc'],true)"), 'Marc darf den vollständigen Claims- und Rekon-Abrechnungsbereich lesen');
assert(revenueApi.includes("if($own==='holger')return in_array($profile,['holger','rekon_holger'],true)") && revenueApi.includes('function rsAllowedProfiles('), 'Holger darf serverseitig ausschließlich seine eigenen Claims- und Rekon-Profile lesen');
assert(revenueApi.includes("svnetUserProfile($user)==='marc'") && revenueApi.includes("'summary_type'=>'marc'") && revenueApi.includes("rsStored('claims_marc')") && revenueApi.includes("rsStored('rekon_marc')"), 'Marcs Kopfzeile wird aus seinen ausgewerteten Claims- und Rekon-Umsätzen gebildet');
assert(revenueApi.includes("$action==='refresh'") && revenueApi.includes('Nur Susanne darf die Umsatzdatei manuell aktualisieren') && revenueApi.includes("$_FILES['workbook']") && settlementPage.includes('Jetzt aktualisieren') && settlementPage.includes("body.append('workbook',workbooks[0],workbooks[0].name)"), 'Susanne kann die gerade gespeicherte Claims-Umsatzdatei über einen eigenen Knopf zuverlässig neu einlesen');
const revenueWorkflow = fs.readFileSync(path.join(root, '../.github/workflows/revenue-summary-refresh.yml'), 'utf8');
const rekonWorkflow = fs.readFileSync(path.join(root, '../.github/workflows/revenue-rekon-refresh.yml'), 'utf8');
assert(revenueApi.includes("$action==='scheduled'") && revenueApi.includes("HTTP_X_SVNET_SCHEDULE_KEY") && revenueApi.includes("hash_equals($expected,$provided)"), 'Der serverseitige Umsatzabgleich besitzt einen geschützten Endpunkt ohne Browser-Sitzung');
assert(revenueApi.includes('RS_SOURCE_URL') && revenueApi.includes('function rsSharePointSource()') && revenueApi.includes('return rsSharePointSource();'), 'Der Server liest die von Susanne tatsächlich bearbeitete SharePoint-/OneDrive-Originaldatei');
assert(revenueApi.includes('function rsEntries(') && revenueApi.includes("'actual_order_gross'") && revenueApi.includes("'regulierer_gross'") && revenueApi.includes("'private_entries'"), 'Der Umsatzabgleich speichert die Buchungszeilen sowie 60/40-Werte netto und brutto');
assert(revenueApi.includes("preg_match('/^20\\d{2}$/',trim((string)$name))") && revenueApi.includes("'available_years'=>"), 'Alle vorhandenen Jahresblätter werden für die historische Abrechnungsansicht eingelesen');
assert(settlementPage.includes('Auszahlung an SV-Büro (60 %)') && settlementPage.includes('Tatsächliche Auftragssumme (100 %)') && settlementPage.includes('Anteil Die Regulierer (40 %)'), 'Die Abrechnungsansicht weist alle drei 60/40-Größen aus');
assert(settlementPage.includes('Auftragsnummer, VN, Ort oder Versicherer') && settlementPage.includes('[row.order_no,row.customer,row.place,row.insurer]'), 'Die Abrechnungssuche berücksichtigt auch den Ort');
assert(settlementPage.includes('km · Betrag netto / brutto') && settlementPage.includes('Arbeitszeit') && settlementPage.includes('Fahrzeit') && settlementPage.includes('optionalPair(row.work_time_net,row.work_time_gross)') && settlementPage.includes('optionalPair(row.travel_time_net,row.travel_time_gross)'), 'Kilometer mit Betrag, Arbeitszeit und Fahrzeit werden getrennt netto und brutto ausgewiesen');
assert(revenueApi.includes("'distance_km'=>rsDistanceKm($sheet,$row)") && revenueApi.includes("'km_amount_net'=>$kmAmount") && revenueApi.includes("'work_time_net'=>$workAmount") && revenueApi.includes("'travel_time_net'=>$travelAmount"), 'Der Umsatzimport übernimmt Kilometer und die drei Betragsbestandteile getrennt aus der Quelle');
assert(revenueApi.includes("'entry_schema_version'=>2"), 'Die neuen Abrechnungsdetails erhalten eine eindeutige Datenstand-Version');
assert(settlementPage.includes('netto / brutto') && settlementPage.includes('Die Bruttowerte enthalten 19 % Umsatzsteuer') && !settlementPage.includes('download='), 'Die Zahlen erscheinen netto und brutto ausschließlich als Portaltabelle ohne Excel-Download');
assert(settlementPage.includes('type="file"') && settlementPage.includes('Gespeicherte Umsatzdatei auswählen') && settlementPage.includes("file.addEventListener('change'") && revenueApi.includes('function rsUploadedSource()'), 'Die manuelle Aktualisierung verarbeitet die von Susanne ausgewählte Originaldatei statt einer veralteten Cloud-Kopie');
assert(settlementPage.includes('settlement-scroll-top') && settlementPage.includes('holger-scroll-top') && settlementPage.includes('rekon-scroll-top') && settlementPage.includes('syncScrollWidths') && settlementPage.includes('connectScroll('), 'Obere synchronisierte horizontale Schiebeleisten machen die rechten Abrechnungsspalten aller Profile jederzeit erreichbar');
assert(settlementPage.includes('href="?profile=christian"') && settlementPage.includes('href="?profile=holger"') && settlementPage.includes("profile=${profile}"), 'Die Abrechnungsseite trennt Christian und Holger über zwei eindeutige Auswahlknöpfe und getrennte Datenprofile');
assert(settlementPage.includes('href="?profile=claims_marc"') && settlementPage.includes('Marc Claims') && settlementPage.includes('renderMarc('), 'Marcs Claims-Umsätze besitzen eine eigene Auswahl und eine eigene Sammelgutschriften-Auswertung');
assert(revenueApi.includes("function rsStored(string $profile='christian')") && revenueApi.includes("rsPersist('holger'") && revenueApi.includes("$profile==='holger'?rsRefreshHolger"), 'Christians und Holgers Abrechnungsdaten werden getrennt gespeichert und aktualisiert');
assert(revenueApi.includes('RS_HOLGER_SOURCE_URL') && revenueApi.includes('function rsHolgerRanges(') && revenueApi.includes('function rsHolgerEntries(') && revenueApi.includes("'office_share_rate'=>0.10"), 'Holgers Jahresbereiche, Gutschriften und 10-Prozent-Abgabe werden aus seiner eigenen Umsatzdatei gelesen');
assert(revenueApi.includes("'actual_order_net'=>$actualNet") && revenueApi.includes("'regulierer_net'=>$reguliererNet") && revenueApi.includes("'our_total_net'=>$billingNet") && revenueApi.includes("'holger_after_total_net'"), 'Holgers Auswertung trennt Auftragssumme, Die Regulierer, SV-Büro und Holgers verbleibenden Anteil');
assert(settlementPage.includes('Claims-Gutschrift') && settlementPage.includes('10 % an SV-Büro') && settlementPage.includes('Gesamt an SV-Büro') && settlementPage.includes('Holger nach Gesamtbelastung'), 'Holgers Buchhaltungsauswertung weist alle Empfänger sowie Office 365 netto und brutto aus');
assert(revenueApi.includes("'average_actual_order_net'") && revenueApi.includes("'settlement_count'=>$count") && settlementPage.includes('Ø Auftragssumme pro Abrechnung') && settlementPage.includes('Abrechnungen)'), 'Holgers Übersicht zeigt den Durchschnitt je Abrechnung und bezeichnet die nicht aufgegliederten Datensätze nicht als Aufträge');
assert(revenueApi.includes("'can_refresh'=>$action==='settlement'") && portalServiceWorker.includes("CACHE_VERSION = '20260905-7'") && portalServiceWorker.includes("self.addEventListener('install', () => self.skipWaiting())") && internLayout.includes("updateViaCache: 'none'"), 'Veraltete Portal-Caches werden ersetzt und die alte Aktualisierung im orangefarbenen Kasten serverseitig unterdrückt');
assert(revenueApi.includes("svnetUserProfile($user)==='holger'") && revenueApi.includes("'summary_type'=>'holger'") && revenueApi.includes("'show_settlement_link'=>true") && revenueApi.includes("'default_profile'=>'holger'") && revenueApi.includes("$claimsCurrent['income_net']") && revenueApi.includes("$rekonCurrent['payout_net']"), 'Holger erhält die Abrechnungsseite ausschließlich für seine gespeicherten Claims- und Rekon-Umsätze');
assert(internLayout.includes("summary.summary_type === 'marc' || summary.summary_type === 'holger'") && internLayout.includes("const person = summary.summary_type === 'marc' ? 'Marc' : 'Holger'") && internLayout.includes('Claims · ${claims.period') && internLayout.includes('Rekon · ${rekon.period'), 'Der orange Kopfbereich zeigt Marc und Holger personenbezogen jeweils genau einen Claims- und einen Rekon-Umsatz');
assert(internLayout.indexOf('class="intern-header__login"') < internLayout.indexOf('class="site-brand"') && !internLayout.match(/<aside class="intern-sidebar">[\s\S]*class="intern-header__login"/), 'Anmelden steht oben links unmittelbar vor dem SV-Logo und nicht mehr in der Seitenleiste');
assert(settlementPage.includes('if(next.default_profile)') && settlementPage.includes('refresh.hidden=!next.can_refresh'), 'Nicht berechtigte Nutzer werden abgewiesen und Rekon-Berechtigte auf ihren zulässigen Startbereich geführt');
assert(settlementPage.includes("next.source_provider||'Datenquelle'"), 'Die Abrechnungsansicht zeigt die tatsächlich verwendete Datenquelle an');
...
assert(revenueApi.includes('RS_MARC_SOURCE_URL') && revenueApi.includes('function rsMarcSharePointSource()') && revenueApi.includes('function rsRefreshMarc(') && revenueApi.includes("rsPersist('claims_marc'") && revenueApi.includes("$saved['claims_marc']"), 'Marcs Claims-Datei wird aus SharePoint automatisch eingelesen und getrennt gespeichert');
assert(revenueApi.includes('rsGoogleSource()') && revenueApi.includes("$errors['christian']"), 'Christians Quellenfehler blockiert Marcs getrennten Umsatzimport nicht und Google Drive bleibt als Rückfallquelle aktiv');
assert(revenueWorkflow.includes("'claims_marc' not in payload.get('profiles', [])"), 'Der geplante Umsatzworkflow bestätigt Marcs tatsächlich gespeichertes Claims-Profil');
assert(revenueApi.includes('function rsMarcWarnings(') && !settlementPage.includes('Hinweis zur Originaldatei'), 'Widersprüchliche Leistungszeiträume bleiben intern erfasst, werden aber nicht als Portalhinweis eingeblendet');
assert(rekonWorkflow.includes("cron: '20 5 15 * *'") && rekonWorkflow.includes('secrets.SETUP_KEY') && rekonWorkflow.includes('action=scheduled_rekon'), 'Der Rekon-Abgleich läuft serverseitig jeweils am 15. des Monats');
assert(revenueApi.includes('function rsRekonSharePointSources(') && revenueApi.includes('Buchhaltung Intern/Rekon/') && revenueApi.includes("preg_match('/einzelposten"), 'Der automatische Rekon-Abgleich liest ausschließlich die geldführenden Einzelpostenlisten aus Katjas SharePoint-Ordnern');
assert(revenueApi.includes('function rsRekonEntries(') && revenueApi.includes("'rekon_net'=>$rekon") && revenueApi.includes("'payout_net'=>$payout") && revenueApi.includes("'actual_order_net'=>$actual"), 'Rekon-Einzelposten werden als 100-Prozent-Auftragssumme, 40-Prozent-Rekon-Anteil und 60-Prozent-SV-Auszahlung gespeichert');
assert(settlementPage.includes('href="?profile=rekon_marc"') && settlementPage.includes('href="?profile=rekon_holger"') && settlementPage.includes('Anteil Rekon (40 %)') && settlementPage.includes('Auszahlung SV-Büro (60 %)'), 'Die Portalansicht enthält getrennte Rekon-Auswertungen für Marc und den künftigen Holger-Bestand');
assert(settlementPage.includes('href="?profile=annual"') && settlementPage.includes('renderAnnual()') && settlementPage.includes('Gesamtsumme') && revenueApi.includes('function rsAnnualPayload()'), 'Die Jahresabrechnung führt alle aktuell eingelesenen Umsatzbereiche in Einzelpositionen und einer Netto-Brutto-Gesamtsumme zusammen');
assert(revenueApi.includes("'profile'=>'rekon_holger'") && revenueApi.includes('Noch keine Holger-Rekon-Datei eingelesen') && revenueApi.includes("'available'=>$available") && revenueApi.includes("$profile==='annual'"), 'Fehlende Holger-Rekon-Daten werden in der Jahresabrechnung ausgewiesen und nicht als Nullumsatz erfunden');
assert(revenueApi.includes('Nur Katja oder Susanne dürfen Rekon manuell aktualisieren') && settlementPage.includes("body.append('workbooks[]'"), 'Katja und Susanne können mehrere aktuelle Rekon-Einzelpostenlisten gemeinsam manuell einlesen');
...
assert(manifest.version === '1.4.11' && !serviceWorker.includes('REVENUE_REFRESH_ALARM') && !serviceWorker.includes('PORTAL_REVENUE_REFRESH') && !bridge.includes('PORTAL_REVENUE_REFRESH'), 'Der Umsatzabgleich darf nicht mehr von einer installierten Browser-Brücke abhängen');
...
assert(serviceWorker.includes('claimsById = new Map()') && serviceWorker.includes('bucketCounts') && serviceWorker.includes('unterschiedliche Aufträge mit und ohne Termin'), 'Aufträge aus beiden Ansichten werden vor dem Delta-Abgleich ohne Dubletten zusammengeführt');
