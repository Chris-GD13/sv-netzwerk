...
assert.equal(manifest.version, '1.3.9', 'die aktuelle ClaimsForce-Anmeldung und der offene Zähler müssen als neue Laufzeitversion erkennbar sein');
...
assert(claimsLogin.includes("button && !button.disabled") && claimsLogin.includes('attempts < 30') && claimsLogin.includes('form.requestSubmit?.()'), 'Die Brücke wartet auf den verzögert aktivierten ClaimsForce-Anmeldebutton und besitzt einen Formular-Fallback');
...
assert(claimsLogin.includes('new MutationObserver(() => fillLogin())') && claimsLogin.includes('setInterval(() => fillLogin(), 1000)'), 'Die Profilanmeldung wird auch nach einem verzögerten ClaimsForce-Neuaufbau erneut ausgeführt');
...
assert(portal.includes('claimsforce-central.js?v=20260829-8'), 'Portal lädt die Claims-Zähleranzeige ohne alten Browsercache');
...
assert(claimsPageBridge.includes('readAllCount') && claimsPageBridge.includes('allCount: readAllCount()'), 'Der in ClaimsForce sichtbare Zähler „Alle“ wird beim Abruf mit ausgelesen');
...
assert(options.includes("OPTIONS_CODE_VERSION = '1.3.9'"), 'Die Optionsseite lädt die aktualisierte entpackte Erweiterung genau einmal neu');
assert(options.includes("OPTIONS_CODE_VERSION = '1.3.9'") && options.includes('chrome.runtime.reload()'), 'Neue entpackte Brückendateien aktivieren ihren Service Worker einmalig selbst');
...
assert(queue.includes('claimsforce_profile_status') && queue.includes("$action==='summary'"), 'Die offenen Claims-Aufträge werden je Profil dauerhaft gespeichert und portalweit bereitgestellt');
...
assert(queue.includes("array_key_exists('openClaims',$result)") && queue.includes("$result['openClaims']??$result['claims']") && queue.includes('ON DUPLICATE KEY UPDATE open_count=VALUES(open_count)'), 'Ein erfolgreicher Import speichert bevorzugt den ClaimsForce-Zähler „Alle“ ohne zusätzlichen Abruf');
...
assert(central.includes('Browser-Brücke 1.3.9 erforderlich') && central.includes("versionAtLeast(bridgeVersion,'1.3.9')"), 'Nur die Brücke mit aktueller Anmeldung und Claims-Zähler darf neue Warteschlangenläufe starten');
...
assert(serviceWorker.includes('openClaims') && serviceWorker.includes('scraped?.allCount'), 'Das Importergebnis übergibt den ClaimsForce-Zähler „Alle“ an das Portal');
