...
assert.equal(manifest.version, '1.1.8', 'grundlegend reparierte Brücke muss als neue Laufzeitversion erkennbar sein');
...
assert(options.includes("OPTIONS_CODE_VERSION = '1.1.8'") && options.includes('chrome.runtime.reload()'), 'Neue entpackte Brückendateien aktivieren ihren Service Worker einmalig selbst');
...
assert(central.includes('Browser-Brücke 1.1.8 erforderlich') && central.includes("versionAtLeast(bridgeVersion,'1.1.8')"), 'Veraltete geladene Erweiterungen dürfen keine neuen Warteschlangenläufe starten');
...
assert(serviceWorker.includes('!currentProfile && previous.claimsToken') && serviceWorker.includes('claimsLoggedProfile: profile'), 'Eine vorhandene unbekannte ClaimsForce-Sitzung wird dem ausdrücklich angeforderten Profil zugeordnet');
