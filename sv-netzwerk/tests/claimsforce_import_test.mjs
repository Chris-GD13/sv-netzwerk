...
assert.equal(manifest.version, '1.3.20', 'die gegen Profilverwechslung, Ausfall der Morgenplanung und fehlenden Umsatzabgleich abgesicherte Brücke muss als neue Laufzeitversion erkennbar sein');
...
assert(options.includes("OPTIONS_CODE_VERSION = '1.3.20'"), 'Die Optionsseite kennzeichnet die aktuelle Brückenversion');
...
assert(central.includes("currentBridgeVersion='1.3.20'") && central.includes('steht als empfohlenes Update bereit'), 'Die aktuelle Brücke muss weiterhin als empfohlenes Update angezeigt werden');
...
assert(manifest.version === '1.3.20' && serviceWorker.includes("const REVENUE_REFRESH_ALARM = 'svnet-revenue-15-30'") && serviceWorker.includes('for (const day of [15, 30])') && serviceWorker.includes("type: 'PORTAL_REVENUE_REFRESH'") && bridge.includes("message?.type === 'PORTAL_REVENUE_REFRESH'"), 'Susannes Brücke aktualisiert die Umsatzliste am 15. und 30. über ihre angemeldete Portalsitzung');
